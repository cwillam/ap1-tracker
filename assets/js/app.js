...
const app = {
	...
	anki: {
		...
		renderBatchSelector() {
			...
			const activeClass = "px-2.5 py-1 rounded-md transition-all text-xs font-mono font-bold bg-blue-600 text-white shadow-sm";
		...
		sendReportEmail() {
			...
			const subject = `[AP1-Lernkarten] Feedback/Fehler: ${p.topicTitle} (${p.cardId})`;
			const bodyLines = [
				"Hallo Christoph,",
				"",
				"ich habe einen Fehler bzw. Feedback zu einer Lernkarte im AP1 Tracker:",
				"",
				"--------------------------------------------------",
				...
				p.feedback,
				"",
				"Gesendet aus dem AP1 Tracker (Web/PWA)",
			];
		...
		try {
			...
			window.addEventListener("keydown", (e) => {
				...
				if (reportModal && !reportModal.classList.contains("hidden")) {
					...
					return; // Während das Melde-Fenster offen ist, keine Lernkarten-Shortcuts auslösen
				}
	...
	toggleBox(boxId) {
		...
		localStorage.setItem("ap1_" + boxId + "_collapsed", isHidden ? "true" : "false");
	},
	...
	resetData() {
		if (confirm("Wirklich ALLE Daten unwiderruflich löschen?")) {
			...
			localStorage.removeItem("ap1_schoolBox_dismissed");
			localStorage.removeItem("ap1_schoolBox_collapsed");
			localStorage.removeItem("ap1_infoBox_dismissed");
			localStorage.removeItem("ap1_infoBox_collapsed");
			location.reload();
		}
	},
	...
	updateStats() {
		try {
			...
			if (globalCountEl) globalCountEl.textContent = learnedCards;
			...
			if (globalAccuracyEl) {
				...
				// Farbe anpassen basierend auf Quote
			...
			const ankiPct = totalAnkiCards > 0 ? Math.round((learnedCards / totalAnkiCards) * 100) : 0;

			const ankiPercentTextEl = document.getElementById("ankiPercentText");
			if (ankiPercentTextEl) {
				ankiPercentTextEl.textContent = `${ankiPct}% (${learnedCards}/${totalAnkiCards})`;
			}
